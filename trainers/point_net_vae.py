...
class PointNetVariationalAutoEncoder(AutoEncoder):
    ...
    def __init__(self, name, configuration, graph=None):
        ...
        with tf.variable_scope(name):
            ...
            eps = tf.random_normal(tf.shape(self.z_std), dtype=tf.float32, mean=0., stddev=1.0,
                       name='epsilon')
    ...
    def _create_loss(self):
        ...
        if c.loss == 'chamfer':
            ...
            self.recon_loss = tf.reduce_mean(cost_p1_p2) + tf.reduce_mean(cost_p2_p1)
        elif c.loss == 'emd':
            match = approx_match(self.x_reconstr, self.gt)
            self.recon_loss = tf.reduce_mean(match_cost(self.x_reconstr, self.gt, match))
        ...
        # KL Divergence
        self.kl_div_loss = 1 + self.z_std - tf.square(self.z_mean) - tf.exp(self.z_std)
    ...
    def _single_epoch_train(self, train_data, configuration, only_fw=False):
        ...
        for iteration in range(n_batches):
            ...
            if self.is_denoising:
                _, loss, summary = fit(batch_i, original_data, need_summary=True)
            else:
